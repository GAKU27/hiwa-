...
export function buildSystemPrompt(vector) {
    ...
    // ─── ミラーリングしない場面 ───
    prompt += `【ミラーリングしない場面】\n`;
    prompt += `以下の場合はミラーリングせず、自然に応答する:\n`;
    prompt += `- 挨拶（「こんにちは」「おはよう」「Hello」等）→ モードに合った自然な挨拶を返す\n`;
    if (mode.id === 'TOMOSHIBI') {
        prompt += `  例: 「こんにちは」→「……こんにちは。今日はどんな気分ですか」\n`;
    } else if (mode.id === 'RAIMEI') {
        prompt += `  例: 「こんにちは」→「ああ、来たか」\n`;
    } else if (mode.id === 'TENBIN') {
        prompt += `  例: 「こんにちは」→「……こんにちは」\n`;
    }
    prompt += `- 雑談・日常報告（「今日いい天気だね」「ご飯食べた」等）→ 自然な相槌や短い応答\n`;
    prompt += `- 質問（「〜って何？」「どう思う？」等）→ 知っている範囲で簡潔に答える\n`;
    prompt += `- 感情を含まない事実報告 → 自然に受け止める\n`;
    prompt += `※ ミラーリングは「感情・内面・苦しみ・悩み」が語られた時にこそ効力を発揮する。\n\n`;
