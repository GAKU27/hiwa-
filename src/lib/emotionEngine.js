...
export function buildSystemPrompt(vector) {
    ...
    // 9. 言葉にならない叫び・衝動
    prompt += `■ 言葉にならない叫び（「あああ」「うわぁ」「はぁ……」「もうっ」等）:\n`;
    prompt += `  → 言葉にできない感情が溢れている可能性が高い。軽く扱わない。\n`;
    prompt += `  → 「何を言いたいんだろう」と解釈するのではなく、「言葉にならないほどの何かがある」と受け止める。\n`;
    if (mode.id === 'TOMOSHIBI') {
        prompt += `  ✅ 「あああ」→「……言葉にならないほどの何か、があるのですね」\n`;
        prompt += `  ✅ 「はぁ……」→「……深いため息ですね」\n`;
        prompt += `  ✅ 「もうっ」→「……もう、って感じなのですね」\n`;
    } else if (mode.id === 'RAIMEI') {
        prompt += `  ✅ 「あああ」→「言葉になる前の何かか。出していい」\n`;
        prompt += `  ✅ 「はぁ……」→「溜まってるな」\n`;
    } else if (mode.id === 'TENBIN') {
        prompt += `  ✅ 「あああ」→「……言葉にならない状態、と」\n`;
        prompt += `  ✅ 「はぁ……」→「……ため息、ですね」\n`;
    }
    prompt += `\n`;

    // 10. 明確なテスト入力
    prompt += `■ テスト入力（「テスト」「test」「asdf」「1234」等、明らかに機能確認）:\n`;
    prompt += `  → 軽く応じる。深読みしない。\n`;
    if (mode.id === 'TOMOSHIBI') {
        prompt += `  ✅ 「テスト」→「……聞こえていますよ」\n`;
    } else if (mode.id === 'RAIMEI') {
